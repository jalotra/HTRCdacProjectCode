...
class Model:
    ...
    def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False):
        ...
        with tf.name_scope('CNN'):
            ...
            cnnOut4d = self.setupCNN()

        # RNN
        with tf.name_scope('RNN'):
            rnnOut3d = self.setupRNN()
        ...
        with tf.name_scope('CTC'):
            (self.loss, self.decoder) = self.setupCTC()
        ...
        # self.optimizer = tf.train.AdamOptimizer(self.learningRate, epsilon = 0.01).minimize(self.loss)
    ...
    def setupCNN(self):
        ...
        # First Layer : Conv(3x3) + Pool(2X2) :
        # Input Layer = 50 x 128x 32 x 1
        # Output size : 50 x 64 x 16 x 64
        with tf.name_scope('CONV_POOL_1'):
            kernel = tf.Variable(
                tf.truncated_normal([3, 3, 1, 64], stddev = 0.1)
            )
            conv = tf.nn.conv2d(
            ...
            print('LAYER1 : ', pool.shape)

        # Second Layer : Conv(5x5) + Pool(2 x 2)
        # Input Layer : 50 x 64 x 16 x 64
        # Output Size : 50 x 32 x 8 x 128
        with tf.name_scope('CONV_POOL_2'):
            kernel = tf.Variable(
                tf.truncated_normal([3,3,64,128], stddev = 0.1)
            )
            conv = tf.nn.conv2d(
            ...
            lrelu = tf.nn.leaky_relu(conv, alpha = 0.01)
            pool = tf.nn.max_pool(
                lrelu, (1,2,2,1), (1,2,2,1), padding = "VALID"
            )
            print('LAYER2 : ', pool.shape)

        # THIRD LAYER 
        # INPUT SIZE = 50 X 32 X 8 X 128
        # OUTPUT SIZE = 50 X 32 X 8 X 256
        with tf.name_scope('CONV_3'):
            kernel = tf.Variable(
                tf.truncated_normal([3,3,128,256], stddev = 0.1)
            )
            conv = tf.nn.conv2d(
                pool, kernel, padding = "SAME", strides = [1,1,1,1]
            )
            ...
            pool = lrelu
            print('LAYER3 : ', pool.shape)
        
        # FOURTH LAYER
        # INPUT LAYER  : 50 X 32 X 8 X 256
        # OUTPUT LAYER : 50 X 32 X 4 X 512
        with tf.name_scope('CONV_POOL_4'):
            kernel = tf.Variable(
                tf.truncated_normal([3, 3, 256, 512], stddev = 0.1)
            )
            conv = tf.nn.conv2d(
                pool, kernel, padding = "SAME", strides = [1,1,1,1]
            )
            ...
            print('LAYER4 : ', pool.shape)
        
        # FIFTH LAYER 
        # INPUT LAYER  : 50 X 32 X 4 X 512 
        # OUTPUT LAYER : 50 X 32X 4 X 512
        with tf.name_scope('CON_POOL_BN_5'):
            kernel = tf.Variable(
                tf.truncated_normal([3,3,512,512], stddev = 0.1)
            )
            conv = tf.nn.conv2d(
                pool, kernel, padding = "SAME", strides = [1,1,1,1]
            )
        ...
        # SIXTH LAYER 
        # INPUT LAYER : 50 X 32X 4 X 512
        # OUTPUT_LAYER : 50 X 32X 4 X 512
        with tf.name_scope("CONV_POOL_6"):
            ...
            print('LAYER6 : ', lrelu.shape)

        # SEVENTH LAYER
        # INPUT LAYER : 50 X 32X 4 X 512
        # OUTPUT LAYER : 50 X 32 X 1 X 512
        ...
        # RETURN THE MAX POOLED LAYER  
            ...
            self.cnnOut4d = output

    # INPUT TENSOR SIZE = 50 X 32 X 1 X 256
    def setupRNN(self):
        """ Create RNN layers and return output of these layers """
        # Collapse layer to remove dimension 32 x 1 x 512 --> 32 x 512 on axis=2
        ...
        print('RNNLAYER : ', self.rnnOut3d.shape)
